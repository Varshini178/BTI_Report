...
public class StrengthAndOpportunities {
	    ...
	    public void checkRadioButton() throws InterruptedException {
	        ...
	        // Scroll down incrementally
	        for (int i = 0; i < 5; i++) { // Adjust iterations based on the page length
	            js.executeScript("window.scrollBy(0, 1000);"); // Scroll down by 1000 pixels
	            Thread.sleep(500); // Short wait between scrolls
	        }

	        
	            // Locate the radio button
	            WebElement grouped = driver.findElement(By.xpath("//label[2]//input[1]"));

	            // Scroll to the radio button to ensure it is in view
	            js.executeScript("arguments[0].scrollIntoView(true);", grouped);
	            Thread.sleep(500); // Optional wait to observe the scroll

	            try {
	            if (grouped.isDisplayed()) {
	                System.out.println("Grouped radio button is displayed");
	            } else {
	                System.out.println("Grouped radio button is not displayed");
	            }
	            ...
	            WebElement unGrouped = driver.findElement(By.xpath("//span[contains(text(),'Ungrouped')]"));
	            if(unGrouped.isDisplayed()) {
	            System.out.println("Ungrouped Radio button is displayed");
	            }
	    }
